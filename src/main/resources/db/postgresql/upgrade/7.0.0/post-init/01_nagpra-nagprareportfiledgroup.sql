...
BEGIN
   ...
   -- If public.collectionobjects_nagpra does not exist, there is nothing to migrate.
   IF (select to_regclass('collectionobjects_nagpra')::text = 'collectionobjects_nagpra') THEN

      -- If public.collectionobjects_nagpra contains 0 records, there is nothing to migrate.
      IF (SELECT count(*) from collectionobjects_nagpra) > 0 THEN
      ...
      ELSE
         ...
         RAISE NOTICE 'collectionobjects_nagpra has 0 records';

      END IF;
   ELSE

      RAISE NOTICE 'collectionobjects_nagpra does not exist';
