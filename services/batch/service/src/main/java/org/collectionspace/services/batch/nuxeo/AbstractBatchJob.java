...
public abstract class AbstractBatchJob extends AbstractBatchInvocable {
	...
	protected UriInfo createUriInfo(String queryString) throws URISyntaxException {
		...
		// Some code in services assumes pathSegments will have at least one element,
		// so add an empty one.

		pathSegments.add(new PathSegmentImpl("", false));

		return createUriInfo(queryString, pathSegments);
	}
